...
function doyToIfc(year, doy) {
  ...
  console.log(`Debug: year=${year}, doy=${doy}, leap=${leap}, adjDoy=${adjDoy}`);
 ...
 * @param {IFCDate} ifc
 * @param {object} locale
 * @returns {string} e.g. 'Monday March 25'
 */
 ...
 *
 * @example
 * toIFC('2026-03-25')           // 'IFC:2026-03-25'
 * toIFC('2026-03-25', 'short')  // 'Sun Mar 25'
 * toIFC('2026-03-25', 'long')   // 'Sunday March 25'
 * toIFC('2026-03-25', 'object') // { year: 2026, month: 3, day: 25, ... }
 * toIFC(new Date())             // 'IFC:2026-03-25'
 * toIFC()                       // today as IFC string
 */
 ...
 *
 * @example
 * toGregorian('IFC:2026-03-25')          // '2026-03-19'
...
function toGregorian(input, format, locale = EN) {
  ...
  if (format === 'short') {
    return `${locale.weekdaysShort[weekday]} ${mShort[gMonth]} ${gDay}`;
  }
  if (format === 'long') {
    return `${locale.weekdaysLong[weekday]} ${mLong[gMonth]} ${gDay}`;
  }
  return iso;
}

module.exports = { toIFC, toGregorian, isLeap, EN };
